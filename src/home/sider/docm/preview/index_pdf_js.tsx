...
class PDFViewer extends React.Component<IProps, IState> {
  ...
  createPDF = () => {
    const { pageNumber, numPages, fileData } = this.state
    if (!fileData) return
    console.log(fileData)
    return (
      <div>
    ...
    return (
      <div id="PDFViewer">
        {/* <input
          ...
          accept=".pdf"
          onChange={this.handleButtonOnChange}
        /> */}
        <div style={{ float: 'right' }}>
          <InputNumber
            onChange={this.onPageNumberInputChange}
            style={{ width: '150px' }}
            placeholder="输入需要跳转的页"
          />
          <Button onClick={this.handleTurnPage} id="numberPage">
            确认跳转
          </Button>
          <Button onClick={this.handleTurnPage} id="pageUp">
            上一页
          </Button>
          <Button onClick={this.handleTurnPage} id="pageDown">
            下一页
          </Button>
        </div>
        {this.createPDF()}
